  ...
  return (
    <main className="flex min-h-screen items-center justify-center px-6 py-12">
      <section className="w-full max-w-3xl rounded-[2rem] border border-border bg-card/90 p-8 shadow-sm backdrop-blur">
        ...
        <h1 className="app-dialog-title mt-4">Request a password reset token.</h1>
        <p className="mt-4 max-w-2xl text-sm leading-7 text-muted-foreground md:text-base">
          Enter your account email. If the user exists, a reset token will be generated for the password replacement step.
        </p>
        <div className="mt-8 grid gap-4">
          ...
          {debugToken ? (
            <p className="text-sm text-amber-700">
              Local debug reset token: <span className="font-semibold">{debugToken}</span>
            </p>
          ) : null}
          <div>
            <button
            ...
            >
              {forgotMutation.isPending ? 'Requesting...' : 'Request reset token'}
            </button>
          </div>
        </div>
        <div className="mt-8">
