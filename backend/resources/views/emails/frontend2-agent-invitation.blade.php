...
<head>
    <meta charset="UTF-8">
    <title>Activate your agent invitation</title>
</head>
<body style="margin: 0; padding: 24px; background: #f8fafc; color: #0f172a; font-family: Arial, sans-serif;">
    <div style="max-width: 640px; margin: 0 auto; background: #ffffff; border-radius: 16px; padding: 32px; border: 1px solid #e2e8f0;">
        ...
        <h1 style="margin: 0 0 16px; font-size: 24px;">Activate your agent invitation</h1>
        <p style="margin: 0 0 16px; line-height: 1.6;">
            Hello {{ $agent->user?->display_name ?? 'there' }},
        </p>
        <p style="margin: 0 0 16px; line-height: 1.6;">
            You have been invited to become an agent in
            <strong>{{ $agent->business?->display_name ?? 'your business' }}</strong>.
        </p>
        <p style="margin: 0 0 16px; line-height: 1.6;">
            Your first assigned program is:
